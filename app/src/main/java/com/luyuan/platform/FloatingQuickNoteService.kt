...
/**
 * 桌面小部件「记一笔」的真输入框：点组件弹出**底部悬浮速记条**（不打开 App），
 * 在桌面上直接打字、回车即存（SYNC_FORMAT 直写共享目录：device=phone, source=manual），
 * 存完自动收起。需要「显示在其他应用上层」权限（Settings.canDrawOverlays，
 * 双击音量下键功能已用过同一权限）。
 */
    ...
    private fun showBubble() {
        ...
        val root = LinearLayout(this).apply {
            ...
            background = GradientDrawable().apply {
                ...
                cornerRadius = 22 * d
                setStroke((1 * d).toInt(), Color.parseColor("#E5E7EB"))
            }
            elevation = 8 * d
        ...
        try {
            ...
            input.postDelayed({
                if (root.isAttachedToWindow && !input.hasWindowFocus()) {
